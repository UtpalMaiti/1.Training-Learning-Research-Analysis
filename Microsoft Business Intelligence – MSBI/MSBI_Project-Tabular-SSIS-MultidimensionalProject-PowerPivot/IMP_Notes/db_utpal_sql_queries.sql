...
--CHAR DATA TYPE IS USED FOR THE STORING THE FIXED LENGTH OF CHARACTERS AND VARCHAR IS USED FOR STORING THE VARIABLE LENGTH OF CHARACTERS

--WHAT IS THE DIFFERENCE BETWEEN VARCHAR AND NVARCHAR DATA TYPES
--BOTH VARCHAR AND NVARCHAR ALLOWS TO STORE STRING VALUES, NUMERIC VALUES AND SPECIAL CHARACTERS BUT INSIDE NVARCHAR COLUMNS WE CAN SOME EXTRA SPECIAL CHARACTERS AND DIFFERENT LANGUAGES DATA ALSO 
...
SELECT * FROM EMPL_LEAVE_DATA

SELECT * FROM DB_PRAGIM.DBO.TBL_EMPLS_BANK_INFO

--TRANSFERRING THE DATA INTO A NEW TABLE
SELECT * INTO TBL_EMPLS_BANK_DETAILS 
FROM DB_MOHAN.DBO.TBL_EMPLS_BANK_INFO
...
SELECT * FROM DB_MOHAN.DBO.TBL_EMPLS_BANK_INFO
...
--3.WHEN WE APPLY DELETE, IT WILL DO TWO OPERATION, IT WILL TAKE THE BACKUP EXISTING TABLE DATA AND KEEPS SQL BUFFER MEMORY AND THEN DELETES THE TABLE DATA
...
USE DB_PRAGIM
SELECT * FROM TBL_EMP_IDENT
SELECT * FROM TBL_DELETE2

DROP TABLE TBL_EMP_IDENT
DELETE FROM TBL_DELETE2
DROP TABLE TBL_DELETE2
...
SELECT * FROM TBL_EMPL_PRIMARY WHERE EMPID=5
SELECT * FROM TBL_EMPLS_LEAVE_HISTORY WHERE EMPID=5
SELECT * FROM TBL_EMPLS_ADDRESS WHERE EMPID=5
...
--SELECT * FROM CTE_DUP WHERE R_NO>1
...
SELECT SUM(SALARY) AS TOTAL_SALARY FROM TBL_EMPL_MASTER
...
INSERT INTO TBL_EMPLOYS_BANK_DATA VALUES (7,786576576575,'MADIWALA','HDFC')
...
USE DB_PRAGIM
SELECT * INTO TBL_EMPLOYEE_MGR FROM DB_YUGANDAR.DBO.TBL_EMPLOYEE_MGR
...
SELECT * FROM DB_MOHAN.DBO.TBL_EMPLS_STR
SELECT LEN(MAX(EMPID))FROM DB_MOHAN.DBO.TBL_EMPLS_STR
SELECT REPLACE(STR(EMPID,5),' ',0) AS EMPID,EMPNAME,SALARY FROM DB_MOHAN.DBO.TBL_EMPLS_STR
...
--ISNULL
USE DB_PRAGIM
SELECT EMPID,EMPNAME,ISNULL(MOBILENUMBER,0) AS MOB_NUMBER,ISNULL(DATE_OF_JOIN,'1/1/1900') AS DOJ FROM TBL_EMPL_MASTER
SELECT EMPID,EMPNAME,COALESCE(MOBILENUMBER,AGE,EMPID,0) AS MOB_NUMBER,COALESCE(DATE_OF_JOIN,'1/1/1900') AS DOJ FROM TBL_EMPL_MASTER
...
--FOR THE SAME SALARIED PERSONS, THE RANK WILL SHARE THE SAME RANK, BUT IT WILL SKIP THE NEXT RANK COMING IN THE SEQUENCE
...
--INORDER TO CREATE INDEX ON A VIEW
...
--USER 1 QUERIES
ALTER DATABASE DB_PRAGIM SET ALLOW_SNAPSHOT_ISOLATION ON
...
-- =============================================
CREATE PROCEDURE [dbo].[USP_DeleteBanner]
...
BEGIN
	...
	SET NOCOUNT ON;
	DECLARE @Updated TABLE( BannerImage VARCHAR(500))

    -- Insert statements for procedure here
	UPDATE [BannerMaster] SET IsActive = 'N'
	OUTPUT deleted.BannerImage INTO @Updated
	WHERE BannerID = @BannerID

	SELECT 1,BannerImage FROM @Updated
